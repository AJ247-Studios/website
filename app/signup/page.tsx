 ...
 * 
 * ACCEPTANCE CRITERIA:
 * - Maximum 3 fields on primary signup (name, email, password OR magic link)
 * - Social auth option (Google) for faster onboarding
 * - Progressive profile completion after signup (skippable)
 * - Autocomplete attributes for browser autofill
 * - Password strength feedback
 * - Clear error states with helpful messages
 * 
 * ANALYTICS HOOKS:
 * - onSignupComplete: Track successful signups
 * - onSignupFail: Track failed attempts with reason
 * - onSocialAuthClick: Track social auth usage
 */
...
export default function SignUpPage() {
  ...
  const getPasswordStrength = (pwd: string): { level: number; label: string; color: string } => {
    ...
    if (pwd.length >= 12 && variety >= 3) return { level: 4, label: "Strong", color: "bg-emerald-500" };
    if (pwd.length >= 8 && variety >= 2) return { level: 3, label: "Good", color: "bg-blue-500" };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      if (authMethod === "magic-link") {
        ...
        // analytics.track("signup_complete", { method: "password" });

        setMagicLinkSent(true); // Show confirmation for email verification
      }
    } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Analytics: Track social auth
      // analytics.track("signup_complete", { method: "google" });
    } catch (err) {
      setErrors({ email: "Failed to connect with Google" });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-md text-center">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              We sent a {authMethod === "magic-link" ? "sign-in link" : "confirmation link"} to{" "}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-800">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Get access to your client portal and files
            </p>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-50 dark:hover:bg-slate-800 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 transition-colors"
          >
          ...
          {/* Auth method tabs */}
          <div className="flex gap-2 p-1 mb-6 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <button
              type="button"
                ...
                {password && (
                  <div className="mt-2">
                    ...
                    <p id="password-hint" className="text-xs text-slate-500 dark:text-slate-400">
                      {passwordStrength.label} {passwordStrength.level < 3 && "• Use 8+ characters with numbers & symbols"}
                    </p>
                  </div>
                )}
              </div>
